...
import { BOOKING_RANGES, type BookingRange } from "@/types/booking";
...
function bookingsHref(status: string, range: string | undefined, page = 1) {
...
export default async function AdminBookingsPage({
  ...
  const data = await getAdminBookings({ status, range, page, pageSize: 25 });
  ...
  return (
    <div className="admin-enter space-y-6">
      ...
      <BookingFilters status={status} range={range} />
      ...
      {data.totalPages > 1 ? (
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex gap-3">
            {data.page > 1 ? (
              <Link
                href={bookingsHref(status, range, data.page - 1)}
